...
import com.sbtn.androidtv.amazon.PurchaseDataSource.PurchaseStatus;
...
public class AmazonIAPManager {
    ...
    final private AmazonPurchaseActivity mainActivity;
    private UserIapData userIapData;
    final private PurchaseDataSource dataSource;

    public AmazonIAPManager(final AmazonPurchaseActivity mainActivity) {
    ...
    public void setAmazonUserId(final String newAmazonUserId, final String newAmazonMarketplace) {
        ...
        if (newAmazonUserId == null) {
            ...
            if (userIapData != null) {
                userIapData = null;
                //FIXME
//                mainActivity.handlePurchaseError(AmazonPurchasingListener.ERROR_NO_USER);
            }
    ...
    public void doPurchaseForSkus(final Map<String, Product> productData) {
        if (productData.containsKey(mainActivity.getSBTNPackage().getProduct_id())) {
            mainActivity.dataOKDoPurchase();
        }
    }
     ...
     *
     */
    public void handlePurchaseOK(String transactionId) {
        //Update SBTN server for package buy OK;
        RequestFramework.updatePayment(mainActivity, mainActivity.getItemId(), mainActivity.getSBTNPackage().getPkd_id(), transactionId,
                new RequestFramework.DataCallBack<DefaultDataCallBackRequest>() {
                    @Override
                    public void onResponse(DefaultDataCallBackRequest dataResult) {
                        mainActivity.handlePurchaseOK();
                    }

                    @Override
                    public void onFailure() {
                        mainActivity.handlePurchaseError(AmazonPurchasingListener.ERROR_PURCHASE_OK_BUT_UPDATE_SERVER_FAILT);
                    }
                });
    ...
    public void handleError(int error) {
        mainActivity.handlePurchaseError(error);
    }
...
//            final MySku mySku = MySku.fromSku(receipt.getSku(), userIapData.getAmazonMarketplace());
            final String mySku = mainActivity.getSBTNPackage().getProduct_id();
            // Verify that the SKU is still applicable.
            if (StringUtils.isEmpty(mySku)) {
     ...
     * @param fromStatus the current purchase status
     * @param toStatus   the new purchase status
     */
     ...
     *
     * @param receiptId the purchase's receipt id
     * @param userId    the user id for the purchase
     */
